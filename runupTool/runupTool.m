...
% *** runup timestack
fname = imageName;%parseFilename(imageName,'noLocal');
...
%         fname.type = 'runup90';
    ...
    gg=1
    %stackData = runup;
    stackData.RAW = fliplr(permute(runup.pixInst(gg).Irgb(:,1:5:end,:), [2 1 3]));
    aa = datetime(runup.t(1:5:end), 'ConvertFrom', 'datenum', 'TimeZone', 'UTC');
    stackData.T = posixtime(aa);
    stackData.XYZ = [flipud(-runup.pixInst(1).X) runup.pixInst(1).Y runup.pixInst(1).Z];


    stackData.RAW(:, 1501:end, :) = [];
    stackData.XYZ(1501:end,:) = [];
...
saveStructure.savename = [inputStructure.imageName(end-33:end-4) '.runup.mat'];%[fname.time '.' fname.when '.' fname.station '.cx.r' fname.type(2:end) '.runup.mat'];
